...
First, the set view of the mappings was obtained then the ```Stream.collect()``` method was applied on the newly created stream from that set which performs a mutable reduction operation on the elements of the stream. A mutable reduction operation collects input elements into a mutable container, such as a ```Collection```, as it processes the elements of the stream.  
...
In the first example, we used ```Entry.getValue()``` as our classification function along with ```Collectors.mapping()``` to apply a reduction operation on the values associated with a given key using a specified downstream collector. Using ```Entry.getKey()``` as our mapping function, we we're able to reduce our data to only use keys as values. Finally, ```Collectors.toList()``` was used as the downstream collector to accept the mapped values.  
